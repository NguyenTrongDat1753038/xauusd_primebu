...
class MTF_EMA_M1_Trigger_Strategy(BaseStrategy):
    ...
    def __init__(self, params):
        ...
        self.m15_ema_separation_threshold = params.get('m15_ema_separation_threshold', 5.0)
    ...
    def get_signal(self, analyzed_data: pd.DataFrame):
        ...
        is_m15_context_bullish = close_price > m15_ema_34 > m15_ema_89 and abs(close_price - m15_ema_89) < self.m15_ema_separation_threshold
        is_m15_context_bearish = close_price < m15_ema_34 < m15_ema_89 and abs(close_price - m15_ema_89) < self.m15_ema_separation_threshold
        ...
        # --- 4. Tạo tín hiệu ---
        if is_uptrend_aligned and is_m15_context_bullish and is_strong_bullish_m1:
            print(f"Tín hiệu MUA (M1 Trigger): HTF Up, M15 Context Bullish, M1 Spike (Body: {m1_body_size:.2f})")
            return 1, None, None

        if is_downtrend_aligned and is_m15_context_bearish and is_strong_bearish_m1:
